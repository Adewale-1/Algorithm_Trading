...
def download_data(currency_pair, start_date, end_date, time_interval, retries=3):
    ...
    while attempt < retries:
        try:
            # Querying the data
            df = yf.download(currency_pair, start=start_date, end=end_date, interval=time_interval, progress=False)

            # Check if the dataframe is empty
            if df.empty:
                raise ValueError(f"No data found for {currency_pair} at {datetime.now()} using interval {time_interval}.")
            ...
            filename = f"{currency_pair} >>> {time_interval} >>> {start_date} >>> {end_date}.csv"
            df.to_csv(filename)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("Usage: python data_collector.py <currency_pair> <start_date> <time_interval>")
        sys.exit("Invalid number of arguments. Exiting.")
    
    currency_pair, start_date, time_interval = sys.argv[1], sys.argv[2], sys.argv[3]
    end_date = datetime.now().strftime('%Y-%m-%d')  # Get the current date as the end date
